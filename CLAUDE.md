...
tests/                       pytest suite
plugin/                      the Obsidian plugin (TypeScript); spawns `ovi --json` -- see below
...
docs/PLUGIN-CONTRACT.md      what `ovi --json` promises a program driving it (the plugin, or a script)
...
Useful flags: `--headless` (never open a window; with a `VAULT_PATH` a missing config is created
from the defaults, without one it raises `ConfigIncompleteError` rather than blocking on a dialog),
`-q/--no-splash`, `-x/--do-not-open`, `-s/--setup` (force the setup screen), `-i/--init` (delete
CONFIG.yaml, batch files and workbooks; prompts first), `-d/--debug-level`. For a program driving
ovi: `--json` (one JSON event per line on stdout, implies `--headless`) and the per-run setting
flags `--skip-folders`, `--max-value-links`, `--max-tag-links`, `--spreadsheet-app` -- the
contract is `docs/PLUGIN-CONTRACT.md`, pinned by `tests/test_json_cli.py` (in process) and
`tests/test_headless_cli.py` (end to end).
...
### First run: the GUI, or a vault path

`CONFIG.yaml` is normally generated by the Tk setup screen. A `--headless` run cannot show it, so
`SysConfig` has a second way in: when `interactive` is False, `force_setup` is False and a
`vault_path_override` was given, `bootstrap_headless()` writes the config from the defaults, that
vault and any `setting_overrides`, in place of the screen. This is the Obsidian plugin's first run.
With no vault there is nothing to build a config around, so `SysConfig(interactive=False)` on its
own still raises `ConfigIncompleteError`, and so does `--setup --headless`; `can_bootstrap_headless()`
is the gate. `tests/test_setting_overrides.py` covers it in process against a temp config with
no obsidian.json; `tests/test_headless_cli.py` runs the real command line in a child process with
an empty home directory and is the one test that exercises the real `SysConfig` end to end.

**Overrides are for one run.** `setting_overrides` (`skip_rel_str`, `link_lim_vals`,
`link_lim_tags`, `sys_pn_wb_exec`) are applied by `apply_setting_overrides()` *after*
`select_vault_by_path()`, because `apply_vault()` copies `skip_rel_str` back out of the vault
record and would undo an earlier write. The per-vault three go into the record in **both**
`cur_vlts` and `sys_vlts` (separate objects after `load_config()`), as the setup screen does, and
then `cfg_pack()` runs. On an existing config nothing is saved: the plugin's settings page must not
rewrite a setup the user completed on the command line.
...
1. **`SysConfig`** (`ovi_setup.py`) — dataclass holding all system + per-vault settings. Reads/writes
   ...
   Tk `SetupScreen` (`ovi_setupscreen.py`) is shown before anything else runs -- or, headless with
   a vault path, `bootstrap_headless()` writes the config instead (see "First run" above).
