...
"SysConfig.sys_pn_batch": "Full path to this run's batch file, allocated sequentially per vault as ovi_<vault>_NNNN.yaml -- or plain ovi_<vault>.yaml when bool_file_seq is off.",
...
"SysConfig.bool_file_seq": "Use filename Sequencing? checkbox. True appends _NNNN to the batch file and workbook names; False writes one unnumbered pair per vault, overwritten by every run. Took over the reserved bool_unused_1, which cfg_unpack() drops from older vault records.",
